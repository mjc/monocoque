...
/// Redacted PLAIN password wrapper.
#[derive(Clone)]
struct PlainPassword(String);
...
impl PlainPassword {
    ...
    /// View the password as plaintext.
...
/// Redacted CURVE secret-key wrapper.
#[derive(Clone)]
struct CurveSecretKeyBytes([u8; 32]);

impl CurveSecretKeyBytes {
    /// Create a new CURVE secret-key wrapper.
...
impl fmt::Debug for CurveSecretKeyBytes {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str("CurveSecretKeyBytes([REDACTED])")
    }
}
...
pub struct SocketOptions {
    ...
    /// - Default: None (no authentication)
    plain_password: Option<PlainPassword>,
    ...
    /// - Default: None (no encryption)
    curve_secretkey: Option<CurveSecretKeyBytes>,
...
impl SocketOptions {
    ...
    pub fn plain_password(&self) -> Option<&str> {
        self.plain_password.as_ref().map(PlainPassword::as_str)
    }
    ...
    pub fn with_plain_credentials(
        ...
        self.plain_password = Some(PlainPassword::new(password));
        self
    }
    ...
    pub const fn with_curve_keypair(mut self, publickey: [u8; 32], secretkey: [u8; 32]) -> Self {
        ...
        self.curve_secretkey = Some(CurveSecretKeyBytes::new(secretkey));
        self
    }
